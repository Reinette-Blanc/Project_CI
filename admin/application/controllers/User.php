...
class User extends CI_Controller{
	...
	public function login(){
		...
		array('required'=>' กรุณากรอก %s','alpha_numeric'=>'%s อนุญาตภาษาอังกฤษและตัวเลขเท่านั้น'));
...
$this->form_validation->set_rules('old_password','รหัสผ่านเดิม ','required',array('required'=>'กรุณากรอก %s'));
		...
		$this->form_validation->set_rules('confirm_password','ยืนยันรหัสผ่าน','required|matches[new_password]',array('required'=>'กรุณากรอก %s','matches'=>'กรุณากรอก
			%s ให้ตรงกับรหัสผ่านใหม่'));
		if($this->form_validation->run()==false){
		...
		$this->load->view("layout/main",$data);
			...
			$ss_admin_id = $this->session->userdata('ss_admin_id');
			$old_password = $this->input->post('old_password');
			$new_password = $this->input->post('new_password');

			$params['pwd'] =  md5($new_password);
			$this->db->where('admin_id',$ss_admin_id);
